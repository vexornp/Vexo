...
- **If you're changing control flow in a hot path, write the integration test first.** "Behavioral simplification, no new tests needed" is a claim that needs evidence. If you're changing how the render loop, event loop, or reconciliation pipeline decides whether to do work, the existing unit tests almost certainly don't cover the multi-frame control-flow path — write an integration test that runs the path end-to-end (e.g., two `render_retain()` calls with an active animation) before removing the old behavior. The always-on display link refactor shipped a navigation stall because the self-rearm it removed was masking a gap in the early-return condition; no existing test exercised frame 2+ of an animation.
...
- **When debugging GUI bugs, always use the `debugging-gui-with-logs` skill.** Follow its workflow strictly: (1) form hypothesis, (2) add `log::debug!` with a unique prefix, (3) hand the user a `tee`-to-`/tmp` run command and have them run the demo and reproduce the bug, (4) read the captured log file yourself, (5) fix root cause. Never skip to theory or try to reason without log evidence first.
