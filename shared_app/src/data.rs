...
/// Avatar source for a conversation. `Bytes` is the legacy solid-color PNG
/// (generated by `make_avatar_png`); `Url` defers to `NetworkImage` for
/// async fetch + decode via `ImageCache`. Only `Conversation` adopts this
/// enum — `Contact` and `Profile` keep `Rc<[u8]>` since they never carry
/// URL avatars.
#[derive(Clone, Debug)]
