...
fn vs_main(
    ...
    let pixel_pos = inst_pos + (model_pos * inst_size);
    ...
    out.border_width = inst_border_width / max(inst_size.x, inst_size.y);
    return out;
}
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // Get the distance to the furthest edge (0.0 at center, 0.5 at edge)
    let edge_dist = max(abs(centered_uv.x), abs(centered_uv.y));

    // Smooth the transition for a cleaner look
    let border_start = 0.5 - in.border_width;
    let is_border = smoothstep(border_start - 0.005, border_start, edge_dist);
