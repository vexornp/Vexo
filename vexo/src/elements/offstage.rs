...
/// Element for `Offstage` — single-child element that persists across `offstage` toggles.
///
/// Structurally identical to `OpacityElement`: the child element is reconciled via
/// `update_child`/`inflate_child`/`unmount_child`. Because `can_update` checks type only
/// (not the `offstage` flag), the child element survives when `offstage` flips — only the
/// render object's `children()` filtering changes, hiding/showing the child subtree.
...
impl Default for OffstageElement {
    fn default() -> Self {
        Self::new()
    }
}

impl RenderObjectElement for OffstageElement {
